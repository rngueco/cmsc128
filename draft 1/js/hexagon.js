...
function hexagon () {
	...
	hex.graphics.click = function(ev) {
		...
		hex.bgColor = 0xeeefff;
		//powers of two
		
		var c = 1;
		var sum = 1;
		var a = "";
		for(var i = 0; i < (hex.indy + 1); i++){
			if(i<hex.indy){
				a += c + " + ";
			} else {
				a += c;
			}
			c = parseInt(c*((hex.indy+1)-(i+1))/(i+1));
			sum = sum + c;
			}
		prompt("The sum of this row is equal to 2 raised to " + hex.indy+" i.e: " + a + " = "+sum);
		
		/**combinatorial**/
		prompt("this cell is the value of "+hex.indy+" taken "+hex.indx);
